...
3.  **(Recommended) UptimeRobot** — set up a free monitor at [https://uptimerobot.com](https://uptimerobot.com) hitting your Render URL every 5 minutes as a third safety net.
...
| `/checkchannel` | Check channel/group detection status |
| `/setchannel -100xxxxx` | Manually set the channel chat ID |
| `/detectchannel` | Guide for detecting the channel ID |
...
| `channel_id.json` | Cached channel chat ID (persists across restarts) |
...
1.  **Add the CORRECT bot as admin** to your Telegram **channel** and **group**
    -   Bot username: **@Lenskartttttbot** (NOT any other "Lenkart" account)
    -   Search for `@Lenskartttttbot` in the admin add dialog
    -   Make sure you add the bot with username `Lenskartttttbot`, not a similarly-named account
2.  **Give the bot "Read all group messages" permission** in the group
3.  **Post a message in the channel** after adding the bot — this triggers auto-detection of the channel ID
4.  **Restart the bot** after adding it as admin
5.  **Verify with `/checkchannel`** — both Channel and Group should show ✅
6.  **Find your Telegram user ID** and add it to `ADMIN_USER_IDS`
7.  **Deploy** to Render or run locally with `python3 run_bot.py`

> **IMPORTANT:** The bot must be added as admin with the exact username **@Lenskartttttbot**. If you added a different "Lenkart" account, the bot won't be able to check membership. Use `/checkchannel` to verify the bot's status.
> 
> **Note:** The bot needs to be an **admin** in both the channel and group so it can use `getChatMember` to verify if users have joined. The channel ID is auto-detected when the bot is added or when any message is posted in the channel, and is saved to `channel_id.json` for persistence across restarts.