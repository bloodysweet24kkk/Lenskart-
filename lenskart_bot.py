...
class LenskartFakeDevice:
    ...
    def send_register_otp(self, email: str, password: str, first_name: str):
        ...
        Uses /v3/customers/register/sendOtp which requires all registration fields.
...
def cmd_start(chat_id):
    ...
    menu_text = (
        ...
        "\U0001f4e7 <b>Email Login</b> \u2014 Login or create new account\n"
...
def _handle_signup_yes(chat_id):
    ...
    Transition the email flow to the signup_phone stage."""
    with _sessions_lock:
...
def handle_email_step(chat_id, text):
    ...
    Flow:
      email -> password -> [try login]
        -> SUCCESS: claim reward
        -> FAIL: signup_phone -> signup_otp -> signup_firstname -> [register] -> claim reward
    """
    with _sessions_lock:
        state = _email_pending.get(chat_id)
    if not state:
    ...
    if state["stage"] == "password":
        ...
        def worker():
            ...
            if status == 200 and device.auth_token:
                # ---- LOGIN SUCCESS ----
                _claim_and_report(chat_id, device, email, password, "email")
                with _sessions_lock:
                    _email_pending.pop(chat_id, None)
            else:
    ...
    # ---- STAGE: signup_phone (login failed, user wants to create account) ----
    ...
    if state["stage"] == "signup_otp":
        ...
        tg_send(chat_id,
                "\U0001f464 OTP received! Send your first name (for the new account):")
        return True
    ...
    if state["stage"] == "signup_firstname":
        ...
        if len(first_name) < 2:
            ...
            return True
        with _sessions_lock:
            state = _email_pending.pop(chat_id, None)
        if not state:
            tg_send(chat_id, "\u274c Session expired. Please /start again.")
            return True
        ...
        def worker():
            ...
            _claim_and_report(chat_id, device, email, password, "signup", phone)
...
def _claim_and_report(chat_id, device, email, password, account_type, phone=None):
    """Claim reward and send the result message. Also save to history."""
