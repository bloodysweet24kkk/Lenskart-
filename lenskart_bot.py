...
_pending_sessions = {}
_email_pending = {}
_signup_pending = {}  # {chat_id: {"stage": ..., "phone": ..., "device": ..., "email": ...}}
...
class LenskartFakeDevice:
    ...
    def register(self, email: str, password: str, first_name: str, otp: str):
        ...
        The phone must already have an OTP sent via send_otp().
...
def cmd_start(chat_id):
    ...
    with _sessions_lock:
        ...
        _signup_pending.pop(chat_id, None)
    ...
    menu_text = (
        ...
        "\U0001f195 <b>Create Account</b> \u2014 Make a NEW Lenskart account\n"
    ...
    buttons = [
        ...
        [{"text": "\U0001f195 Create Account", "callback_data": "menu_signup"}],
...
def cmd_menu_signup(chat_id):
    """User pressed the Create Account button — start signup flow."""
    with _sessions_lock:
        _pending_sessions.pop(chat_id, None)
        _email_pending.pop(chat_id, None)
        _menu_pending.pop(chat_id, None)
        _signup_pending[chat_id] = {"stage": "phone", "phone": None, "device": None, "email": None}
    tg_send(chat_id,
            "\U0001f195 <b>Create New Lenskart Account</b>\n"
            "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n"
            "Steps to create a new account:\n"
            "1\ufe0f\u20e3 Send a 10-digit phone number (OTP will be sent to it)\n"
            "2\ufe0f\u20e3 Enter the OTP you receive\n"
            "3\ufe0f\u20e3 Enter email, password & name\n\n"
            "\U0001f4f1 Send the phone number for the new account\n"
            "Example: <code>9876543210</code>")
...
def cmd_cancel(chat_id):
    with _sessions_lock:
        ...
        d = _signup_pending.pop(chat_id, None)
    if a or b or c or d:
...
def handle_signup_step(chat_id, text):
    """Handle the multi-step signup conversation:
    phone -> otp -> email -> password -> first_name -> register -> claim reward"""
    with _sessions_lock:
        state = _signup_pending.get(chat_id)
    if not state:
        return False  # not in signup flow
    ...
    if state["stage"] == "phone":
        ...
        if not res:
            ...
            with _sessions_lock:
                _signup_pending.pop(chat_id, None)
            return True
        is_new = res.get("isNewUser")
        with _sessions_lock:
            _signup_pending[chat_id]["phone"] = phone
            _signup_pending[chat_id]["device"] = device
            _signup_pending[chat_id]["stage"] = "otp"
        if is_new:
            tg_send(chat_id,
                    f"\U0001f4e8 OTP sent to <code>{phone}</code>\n"
                    f"\U0001f195 New account will be created.\n\n"
                    f"Send the OTP you received:")
        else:
            tg_send(chat_id,
                    f"\U0001f4e8 OTP sent to <code>{phone}</code>\n"
                    f"\u2139\ufe0f This number already has an account.\n\n"
                    f"Send the OTP you received:")
        return True
    ...
    if state["stage"] == "otp":
        ...
        if len(otp) < 4:
            ...
            return True
        with _sessions_lock:
            _signup_pending[chat_id]["otp"] = otp
            _signup_pending[chat_id]["stage"] = "email"
        tg_send(chat_id,
                "\U0001f4e7 OTP received! Now send the email address for the new account:")
        return True
    ...
    if state["stage"] == "email":
        if "@" not in text or "." not in text.split("@")[-1]:
            ...
            return True
        with _sessions_lock:
            _signup_pending[chat_id]["email"] = text
            _signup_pending[chat_id]["stage"] = "password"
        tg_send(chat_id,
                "\U0001f511 Send a password for the new account:")
        return True

    # ---- STAGE: password ----
    if state["stage"] == "password":
        if len(text) < 4:
            tg_send(chat_id, "\u26a0\ufe0f Password too short. At least 4 characters. Try again or /cancel.")
            return True
        with _sessions_lock:
            _signup_pending[chat_id]["password"] = text
            _signup_pending[chat_id]["stage"] = "firstname"
        tg_send(chat_id,
                "\U0001f464 Send your first name (for the account):")
        return True
    ...
    if state["stage"] == "firstname":
        first_name = text
        if len(first_name) < 2:
            tg_send(chat_id, "\u26a0\ufe0f Please send a valid name. Try again or /cancel.")
            return True

        # Gather all stored data
        with _sessions_lock:
            state = _signup_pending.pop(chat_id, None)
        ...
        tg_send(chat_id, "\U0001f195 Creating your new Lenskart account, please wait...")

        def worker():
            res, status = device.register(email, password, first_name, otp)
            if status != 200 or not device.auth_token:
                msg = res.get("message") if isinstance(res, dict) else "Registration failed"
                tg_send(chat_id, f"\u274c Account creation failed: {msg}")
                return
            tg_send(chat_id, f"\u2705 Account created successfully! Email: {email}")
            ...
            if rres.get("giftVoucher"):
                ...
                msg = f"\u26a0\ufe0f {email}: Account created but reward not unlocked."
            tg_send(chat_id, msg)
        ...
        while True:
            ...
            for u in updates:
                ...
                if cb:
                    ...
                    elif cb_data == "menu_signup":
                        cmd_menu_signup(cb_chat_id)
                ...
                if text == "/start":
                    ...
                    with _sessions_lock:
                        ...
                        pending_signup = chat_id in _signup_pending
                        menu_state = _menu_pending.get(chat_id)
                    if pending_signup:
                        handle_signup_step(chat_id, text)
                    elif pending_email:
