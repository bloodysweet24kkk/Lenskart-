...
#  CONFIG  --  EDIT THESE
# ============================================================
BOT_TOKEN = "8847511389:AAHUGxyLekuFi7swno71ANXW3A8IrNHEyRg"
...
CHANNEL_ID_FILE = _os.path.join(_DATA_DIR, "channel_id.json")  # persistent channel ID cache
...
def _load_saved_channel_id():
    """Load the previously auto-detected channel ID from file (survives restarts)."""
    try:
        if _os.path.exists(CHANNEL_ID_FILE):
            with open(CHANNEL_ID_FILE, "r") as f:
                data = json.load(f)
                cid = data.get("channel_chat_id")
                if cid:
                    logger.info(f"Loaded saved channel ID from file: {cid}")
                    return cid
    except Exception as e:
        logger.warning(f"Failed to load saved channel ID: {e}")
    return None


def _save_channel_id(chat_id):
    """Persist the detected channel ID so it survives bot restarts."""
    try:
        with open(CHANNEL_ID_FILE, "w") as f:
            json.dump({"channel_chat_id": chat_id}, f)
        logger.info(f"Saved channel ID to file: {chat_id}")
    except Exception as e:
        logger.warning(f"Failed to save channel ID: {e}")
...
def _resolve_channel_id():
    ...
    if CHANNEL_CHAT_ID:
        ...
        return _resolved_channel_id
    # Try to load from persistent file (survives restarts)
    saved = _load_saved_channel_id()
    if saved:
        _resolved_channel_id = saved
        return _resolved_channel_id
    # If still not found, the bot will auto-detect it when it receives
    # a channel_post or my_chat_member update (bot added as admin to channel)
    return None
...
def tg_get_updates(offset: int, timeout: int = 30):
    try:
        r = requests.get(
            ...
            params={"offset": offset, "timeout": timeout, "allowed_updates": json.dumps(["message", "callback_query", "channel_post", "chat_member", "my_chat_member"])},
            timeout=timeout + 10,
        )
        if r.status_code == 200:
        ...
        def worker():
            ...
            if not allowed:
                ...
                with _sessions_lock:
                    _email_pending.pop(chat_id, None)
                return
            tg_send(chat_id, f"\U0001f510 Device verified \u2705\nLogging in with email...")
...
def handle_admin_command(chat_id, text, user_id):
    ...
    elif cmd == "/setchannel" and len(parts) >= 2:
        """Manually set the channel chat ID (e.g. /setchannel -1001234567890)"""
        try:
            new_cid = int(parts[1])
            _resolved_channel_id = new_cid
            _save_channel_id(new_cid)
            ...
            try:
                ...
                if r.status_code == 200 and r.json().get("ok"):
                    title = r.json()["result"].get("title", "unknown")
                    tg_send(chat_id, f"\u2705 Channel ID set to <code>{new_cid}</code>\nChannel: <b>{title}</b>\nForce-join is now ACTIVE for this channel!")
                else:
                    tg_send(chat_id, f"\u26a0\ufe0f Channel ID set to <code>{new_cid}</code> but bot cannot access it. Make sure bot is admin of the channel.")
            except Exception as e:
                tg_send(chat_id, f"\u26a0\ufe0f Channel ID saved as <code>{new_cid}</code> (verification failed: {e})")
        except ValueError:
            tg_send(chat_id, "\u274c Invalid channel ID. Usage: /setchannel -1001234567890")
        return True

    elif cmd == "/checkchannel":
        """Show current channel/group detection status"""
        cid = _resolve_channel_id()
        gid = _resolve_group_id()
        msg = "\U0001f4f0 <b>Channel/Group Status</b>\n"
        msg += "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n"
        if cid:
            msg += f"\U0001f4f0 Channel ID: <code>{cid}</code> \u2705\n"
        else:
            msg += "\U0001f4f0 Channel ID: <b>NOT DETECTED</b> \u274c\n"
            msg += f"   Invite: {CHANNEL_INVITE_LINK}\n"
            msg += "   Bot must be admin of channel + a message must be posted\n"
        if gid:
            msg += f"\U0001f465 Group ID: <code>{gid}</code> \u2705\n"
        else:
            msg += f"\U0001f465 Group ID: <b>NOT DETECTED</b> \u274c\n"
            msg += f"   Group: @{GROUP_USERNAME}\n"
        # Check if bot is admin of group
        try:
            r = requests.post(f"{TELEGRAM_API}/getChatMember", json={"chat_id": f"@{GROUP_USERNAME}", "user_id": 8847511389}, timeout=10)
            if r.status_code == 200 and r.json().get("ok"):
                status = r.json()["result"]["status"]
                msg += f"\U0001f916 Bot group status: <b>{status}</b>\n"
        except Exception:
            pass
        msg += "\n\u2705 = working | \u274c = needs fix"
        tg_send(chat_id, msg)
    ...
    elif cmd == "/detectchannel":
        """Instructions for detecting the channel ID"""
        msg = (
            "\U0001f4f0 <b>Channel Detection Guide</b>\n\n"
            "Bot ko channel ka admin banane ke baad, channel ID auto-detect karne ke liye:\n\n"
            "<b>Method 1 (Automatic):</b>\n"
            "1. Channel mein ek message post karo\n"
            "2. Bot ko channel_post update milega\n"
            "3. Channel ID auto-detect ho jayega\n\n"
            "<b>Method 2 (Manual):</b>\n"
            "Channel ka chat ID nikalo aur command bhejo:\n"
            "<code>/setchannel -1001234567890</code>\n\n"
            "<b>Channel ID kaise pata kare?</b>\n"
            "@userinfobot ya @RawDataBot ko channel mein add karo,\n"
            "wo channel ID bata dega. Fir us ID ko /setchannel se set karo.\n\n"
            "<b>Current status:</b> /checkchannel bhejo"
        )
        tg_send(chat_id, msg)
        return True
...
def main():
    ...
    # Flush pending updates — but FIRST extract any channel/chat_member updates
    # so we don't lose the channel ID detection
    offset = 0
    ...
    while True:
        ...
        if not stale:
            break
        for su in stale:
            # Check for channel_post to auto-detect channel ID
            cp = su.get("channel_post")
            if cp:
                post_chat = cp.get("chat", {})
                if post_chat.get("type") == "channel" and post_chat.get("id"):
                    if not _resolved_channel_id:
                        _resolved_channel_id = post_chat["id"]
                        _save_channel_id(post_chat["id"])
                        print(f"   \u2705 Channel ID detected during flush: {post_chat['id']}", flush=True)
            # Check for my_chat_member / chat_member updates
            cm = su.get("chat_member") or su.get("my_chat_member")
            if cm:
                cm_chat = cm.get("chat", {})
                if cm_chat.get("type") == "channel" and cm_chat.get("id"):
                    if not _resolved_channel_id:
                        _resolved_channel_id = cm_chat["id"]
                        _save_channel_id(cm_chat["id"])
                        print(f"   \u2705 Channel ID detected from chat_member during flush: {cm_chat['id']}", flush=True)
        offset = stale[-1]["update_id"] + 1
        print(f"   flushed {len(stale)} old update(s)", flush=True)
    # Re-check if we found the channel ID during flush
    if _resolved_channel_id:
        print(f"\u2705 Channel ID now resolved: {_resolved_channel_id}", flush=True)
                ...
                if channel_post:
                    ...
                    if post_type == "channel" and post_chat_id:
                        if not _resolved_channel_id:
                            ...
                            _save_channel_id(post_chat_id)  # persist for restarts
                ...
                if chat_member_update:
                    ...
                    if cm_type == "channel" and cm_chat_id:
                        if not _resolved_channel_id:
                            ...
                            _save_channel_id(cm_chat_id)  # persist for restarts
